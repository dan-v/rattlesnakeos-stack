...
setup_env() {
  ...
  # get core repo
  rm -rf "${CORE_DIR}" || true
  retry git clone "${CORE_URL}" "${CORE_DIR}"
  source "${CORE_DIR}/patches/core_aosp_pre_build.sh"
  source "${CORE_DIR}/patches/core_chromium_pre_build.sh"

  # get custom repo
  rm -rf "${CUSTOM_DIR}" || true
  retry git clone "${CUSTOM_URL}" "${CUSTOM_DIR}"
  source "${CUSTOM_DIR}/patches/aosp_pre_build.sh" || true
  source "${CUSTOM_DIR}/patches/chromium_pre_build.sh" || true
...
EOF
  ...
  log "Running core_chromium_pre_build"
  core_chromium_pre_build
  log "Running chromium_pre_build"
  chromium_pre_build || true
